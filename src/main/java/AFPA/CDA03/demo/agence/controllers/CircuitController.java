...
import AFPA.CDA03.demo.agence.dao.InterfaceCircuitDao;
import java.net.URI;
...
@GetMapping("/circuits")
    //liste des produits
    public List<Circuits> listeCircuits() {
        return Circuits.findAll();
    } 
...
@GetMapping("/circuits/{id}")
    //détail d'un produit
    public Circuits listeCircuits(@PathVariable int id) {
        return Circuits.findById(id); 
    }
    //ajout d’un produit
    ...
    */
    public ResponseEntity<Void> ajouterCircuit(@RequestBody Circuits c) {
        Circuits newCircuit = Circuits.save(c);
    ...
    public void deleteCircuits(@PathVariable int id) {
        Circuits.deleteById(id); }
    //modification d’un circuit
    @PutMapping("/circuits") 
    public void modifierCircuit(@RequestBody Circuits c) { 
        Circuits.save(c);
    }    
} 
